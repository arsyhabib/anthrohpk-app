...
# ===============================================================================
# SECTION 10C: BACKEND LOGIC (CLOUD STORAGE & DATABASE)
# ===============================================================================

# Pastikan library ini sudah diinstall: gspread, oauth2client, pandas, plotly
import gspread
import pandas as pd
import plotly.express as px
from oauth2client.service_account import ServiceAccountCredentials
...
def get_google_sheet_client():
    ...
    try:
        ...
        if not os.path.exists(GOOGLE_CREDS_FILE):
            print(f"⚠️ File {GOOGLE_CREDS_FILE} tidak ditemukan. Fitur Cloud non-aktif.")
            return None
...
def save_analysis_to_cloud(payload):
    ...
    try:
        sheet = client.open(SHEET_NAME)
        # Coba akses tab 'logs', jika error (belum ada), buat baru secara otomatis
        try:
            worksheet = sheet.worksheet("logs")
        except gspread.exceptions.WorksheetNotFound:
            worksheet = sheet.add_worksheet(title="logs", rows=1000, cols=20)
            # Buat Header jika tab baru dibuat
            header = ["Timestamp", "Fitur", "Gender", "Umur (Bulan)", "BB (kg)", "TB (cm)", "LK (cm)", "WAZ", "HAZ", "WHZ"]
            worksheet.append_row(header)

        # Siapkan data (Privasi terjaga: Tanpa Nama)
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        z_scores = payload.get('z', {})
        
        row_data = [
            timestamp,
            "Kalkulator Gizi",              # Nama Fitur
            payload.get('sex', '-'),        # Gender Code (M/F)
            payload.get('age_mo', 0),       # Umur Bulan
            payload.get('w', 0),            # Berat Badan
            payload.get('h', 0),            # Tinggi Badan
            payload.get('hc', 0),           # Lingkar Kepala
            format_zscore(z_scores.get('waz')), # Skor Z Berat/Umur
            format_zscore(z_scores.get('haz')), # Skor Z Tinggi/Umur
            format_zscore(z_scores.get('whz'))  # Skor Z Berat/Tinggi
        ]
        
        worksheet.append_row(row_data)
        print("✅ Data analisis berhasil tersimpan di Cloud")
...
def submit_feedback_to_cloud(performa, manfaat, profesi, kendala, saran):
    ...
    try:
        sheet = client.open(SHEET_NAME)
        # Coba akses tab 'feedback', buat baru jika belum ada
        try:
            worksheet = sheet.worksheet("feedback")
        except gspread.exceptions.WorksheetNotFound:
            worksheet = sheet.add_worksheet(title="feedback", rows=1000, cols=10)
            header = ["Timestamp", "Performa", "Manfaat", "Profesi", "Kendala", "Saran"]
            worksheet.append_row(header)
