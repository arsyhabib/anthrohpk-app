...
def generate_checklist_payload(payload):
    ...
    if os.path.exists("data/imunisasi_schedule.json"):
        ...
        for vaksin in schedule:
            if vaksin['usia'] <= age_mo:
                imun_due.append(f"✅ {vaksin['vaksin']} (usia {vaksin['usia']} bln)")
            elif vaksin['usia'] == age_mo + 1:
                imun_due.append(f"📅 {vaksin['vaksin']} (usia {vaksin['usia']} bln - BULAN DEPAN)")
    ...
    warnings.append("⚠️ RUJUK SEGERA jika tanda bahaya muncul!")
...
def build_markdown_report(name_child, name_parent, age_mo, age_days, sex_text, w, h, hc, z, pct, classifications, lang_mode, errors, warns):
    overall_status = "🟢 Normal"; critical_issues = []
    for k, z_val in z.items():
        if z_val is not None and not math.isnan(z_val):
            if abs(z_val) > 3: overall_status = "🔴 Perlu Perhatian Segera"; critical_issues.append(k.upper())
            elif abs(z_val) > 2 and overall_status == "🟢 Normal": overall_status = "🟡 Perlu Monitoring"
    ...
    for label, key in indices:
        z_val = fmtz(z[key]); pct_val = f"{pct[key]}%" if pct[key] is not None else "—"
...
def generate_parent_narrative(z_scores, perm_class):
    ...
    if z_scores.get('haz') is not None and not math.isnan(z_scores['haz']):
        if z_scores['haz'] < -3: bullets.append("🔴 **Tinggi badan sangat pendek** - Stunting berat"); advice.append("→ **Segera konsultasi**")
        elif z_scores['haz'] < -2: bullets.append("🟡 **Tinggi badan pendek** - Indikasi stunting"); advice.append("→ Konsultasi program perbaikan gizi")
    if z_scores.get('whz') is not None and not math.isnan(z_scores['whz']):
        if z_scores['whz'] < -3: bullets.append("🔴 **Sangat kurus** - Gizi buruk"); advice.append("→ **Butuh penanganan segera**")
        elif z_scores['whz'] < -2: bullets.append("🟡 **Kurus** - Perlu perbaikan gizi"); advice.append("→ Tingkatkan asupan & pantau berat")
        elif z_scores['whz'] > 3: bullets.append("🔴 **Obesitas**"); advice.append("→ Konsultasi ahli gizi")
        elif z_scores['whz'] > 2: bullets.append("🟡 **Berat berlebih**"); advice.append("→ Perhatikan pola makan & aktivitas")
...
String(120, y, "Z-score"); c.drawString(180, y, "Persentil"); c.drawString(250, y, "Status (Permenkes)"); c.drawString(400, y, "Status (WHO)")
        ...
        for key, label in [('waz','WAZ (BB/U)'),('haz','HAZ (TB/U)'),('whz','WHZ (BB/TB)'),('baz','BAZ (IMT/U)'),('hcz','HCZ (LK/U)')]:
            ...
            c.drawString(180, y, f"{pct_val}%" if pct_val is not None else "—")
...
def generate_monthly_checklist(payload):
    ...
    return f"""
# 📋 Checklist Sehat Bulanan - {checklist['usia']} Bulan

## 📊 Status Pertumbuhan: **{checklist['growth_status']}**
{checklist['feeding']}

## 🧠 KPSP: {checklist['kpsp_result']}
**Jadwal Imunisasi:**
{checklist['imunisasi']}
...
with gr.Blocks(
    ...
    ### 📋 **BARU!** Checklist Sehat Bulanan + KPSP + Imunisasi (0-24 bulan)
    ...
    with gr.Tabs():
        ...
        with gr.TabItem("🧮 Kalkulator WHO"):
            with gr.Row():
        ...
        with gr.TabItem("📋 Checklist Bulanan"):
            ...
            with gr.Row():
                with gr.Column(scale=3):
                    ...
                    kpsp_btn = gr.Button("🧠 Hitung KPSP", variant="primary")
